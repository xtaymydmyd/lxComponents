    ...
    methods : {
        ...
        */
        open(config) {
            ...
            if(this.config.model == 1 && !this.config.deptList) {
                common.toastMsg('参数出错');
                return; 
            }
            if(this.config.model == 1 && this.config.deptList.length < 1){
                common.toastMsg('参数出错');
                return; 
            }
                ...
                if(!status){
                    ...
                    for(var i = 0 ; i < this.result.length ; i++){
                        if(item.id == this.result[i].id){
                            this.$emit("on-delete" , JSON.parse(JSON.stringify(this.result[i])));
        ...
        */
        deleteResult(index){
            this.$emit("on-delete" , JSON.parse(JSON.stringify(this.result[index])));
